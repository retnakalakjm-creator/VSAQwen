...
| `TEST` | `evidence/demand.py::_collect_test` | YES | Detector semantics frozen / M12 PARKED; production-active | Primary confirmation | Bullish | Professional demand-map score `0.0`; emitted `Evidence.weight` remains dynamic metadata; directional VSA confirmation remains active | M12 retained the audited five-clause identity including the mandatory strong-downtrend contradiction gate, verified the three confirmations remain diagnostic/non-gating, confirmed distinctness from `NO_SUPPLY` and recovery-anchored `SHAKEOUT`, and found no production detector correction. |
