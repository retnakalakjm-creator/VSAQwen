...
| `INCREASING_SUPPLY` | `evidence/supply.py::_collect_increasing_supply` | YES | **Active / audit-complete** | Primary weakness / supply | Bearish | **Registry 0.85 / configured map 0.70 / runtime 1.00** | Frozen audit population: 528 emissions from 1,022 cheap candidates across 8 symbols. Semantic audit PASS; 0 duplicate emissions; production-path readiness PASS. Decision-value: 63.45% positive decisive rate vs 60.79% eligible market (+2.66 pp), but mean 8-bar return +3.06% vs +3.83% market (−0.77 pp). Weight sensitivity confirmed real score/ranking impact but no qualification/actionability change. Empirical 0.85 is reference-only; runtime remains 1.00. No interaction penalty justified. |
...
| `SUPPLY_DRYING_UP` | `evidence/supply.py::_collect_supply_drying_up` | YES | Active | Supporting / exhaustion context | Contextual | Existing | Production-active / audit-complete |
| `UPTHRUST` | `evidence/supply.py::_collect_upthrust` | YES | **Production-active / audit-complete** | Supply / distribution / trap | Bearish | **Registry 1.00 / professional supply-map weight 0.90 / dynamic runtime** | Mandatory: Buying Campaign, Bullish Bar, Very High Volume, Above-Average Spread. Confirmations: Wide Spread, Weak Close, Lower Close Than Previous; confirmations are non-mandatory. Candidate audit: 1,319 cheap candidates → 289 production emissions; semantic failures 0; duplicate emissions 0. Decision-value: 59.03% positive decisive rate vs 60.80% eligible market (-1.77 pp); mean 8-bar return +2.81% vs +3.83% market (-1.02 pp). Interaction: BUYING_CLIMAX on all 289 events; INCREASING_DEMAND on 224 events, of which 212 are the pure `UPTHRUST + BUYING_CLIMAX + INCREASING_DEMAND` combination. Pure interaction outcome: 56.87% positive decisive rate / +2.27% mean return versus 66.15% / +4.56% for BUYING_CLIMAX-only reference. Counterfactual supply penalties 0.02–0.10 changed the 212 interaction scores and rankings, but moved `net_strength` in the wrong direction because current scoring already reflects the interaction as weakness. **No production penalty. No global UPTHRUST weight change.** Production readiness: PASS; runtime Evidence.weight 0.80–2.00, mean 1.2194. |
| `NO_DEMAND` | `evidence/supply.py::_collect_no_demand` | YES | **Production-active / audit-complete** | Demand absence / weakness | Bearish | **Registry 1.00 / configured map 0.60 / dynamic runtime** | Mandatory: Bullish Environment, Bullish Bar, Low Volume, Narrow Spread. Confirmations: Volume Decreasing, Weak Close; confirmations are non-mandatory. Audit: 109 emissions from 202 cheap candidates; semantic failures 0; duplicate emissions 0. Decision-value: 63.30% positive decisive rate vs 60.79% eligible market (+2.51 pp); mean 8-bar return +3.62% vs +3.83% market (−0.21 pp). Same-bar interaction: 1/109 (0.92%), `SHAKEOUT` only; no supply conflict; no interaction penalty/rejection justified. Weight sensitivity: configured scoring weight 0.60 affects score/ranking, but no qualification/actionability changes were observed. Runtime `Evidence.weight` is dynamic and observed 0.70–1.50; it is not required to equal the configured scoring-map weight. Production runtime weight change: NONE. |
...
| `NO_SUPPLY` | `evidence/demand.py::_collect_no_supply` | YES | **Production-active / audit-complete / contextual-non-scoring** | Demand absence / weakness | Bullish | **Registry 1.00 / scoring map none / dynamic runtime** | Mandatory: Bullish Environment label using production predicate `ctx.is_bearish_environment()`, Bearish Bar, Low Volume, Narrow Spread. Confirmations: Volume Decreasing, Weak Spread, Weak Selling Result; confirmations are non-mandatory. Audit: 23 production emissions from 225 cheap candidates; semantic failures 0; duplicate emissions 0. Decision-value: 60.87% positive decisive rate vs 60.80% eligible market (+0.07 pp); mean 8-bar return +1.02% vs +3.83% market (-2.81 pp); candidate share 0.20%. Same-bar interaction: `SUPPLY_DRYING_UP` on all 23 events, `TEST` on 4; no production penalty or rejection justified. Weight sensitivity: **NOT APPLICABLE** because `NO_SUPPLY` has no entry in either professional scoring map. Runtime `Evidence.weight` remains dynamic and observed 0.90-1.50. Production change: NONE. |
...
### UPTHRUST + INCREASING_DEMAND

`UPTHRUST + BUYING_CLIMAX + INCREASING_DEMAND` is a materially weaker
historical outcome population than `UPTHRUST + BUYING_CLIMAX` alone.

However, the current professional scoring model already produces negative
net-strength for the interaction population. Counterfactual deductions from
the supply score move net-strength toward zero, which is directionally opposite
to the intended effect of a penalty.

Therefore:

- interaction is retained as diagnostic evidence;
- no explicit interaction penalty is introduced;
- no global UPTHRUST weight change is introduced;
- the relationship remains a documented empirical observation.
...
- 547 cheap candidates → 225 production emissions

Production semantics:
- DOWN BAR + LOW VOLUME + NARROW SPREAD
- mandatory semantic failures = 0

Decision value:
- 61.78% positive decisive rate
- eligible market = 60.79%
- lift = +0.99 pp

Mean 8-bar return:
- +3.56%
- eligible market = +3.83%
- lift = -0.26 pp
...
- TEST: 69.77% / +2.55%
- NO_SUPPLY: 63.16% / +0.45%
- NO_SUPPLY + TEST: 50.00% / +3.73%
...
Runtime Evidence.weight:
- 1.00–1.00, mean 1.00
- Professional supply-map weight = 0.60

### SUPPLY_DRYING_UP audit decision

SUPPLY_DRYING_UP is production-valid and shows modest positive hit-rate
lift versus the eligible market, but slightly lower mean-return magnitude.
TEST improves hit rate but reduces mean return; NO_SUPPLY produces a modest
hit-rate improvement with materially weaker follow-through. These interactions
remain diagnostic/study-only. No production scoring bonus, penalty, or rejection
rule is introduced.
