...
def _has_increasing_demand(evidence) -> bool:
    return any(
        str(item.code.value) == "increasing_demand"
...
def _scan_cases(
    ...
    for index in range(scanner.MIN_REPLAY_BARS, len(metrics)):
        ...
        if index < sample_start or index + min(horizons) >= len(metrics):
            continue
        if not _has_increasing_demand(evidence):
            continue
...
def build_matches(
    all_cases: list[tuple[AuditCase, bool]],
    *,
    score_band: float = 0.10,
    pressure_band: float = 0.25,
    max_age_gap: int = 1,
) -> list[MatchedPair]:
    targets = [
        case for case, changed in all_cases
        if changed and case.change in {"True->False", "False->True"}
    ]
