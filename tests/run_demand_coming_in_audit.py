...
def _audit_symbol(
    ...
    for index in range(sample_start, len(metrics)):
        ...
        evidence = collect_demand_coming_in(ctx)
        if not evidence:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Point-in-time DEMAND_COMING_IN audit.")
    ...
    print("=== DEMAND_COMING_IN POINT-IN-TIME AUDIT ===")
