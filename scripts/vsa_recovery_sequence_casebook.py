...
MODULE_NAME = "_pro_vsa_recovery_sequence_casebook"


def _load_casebook_module() -> Any:
    repo_root = Path(__file__).resolve().parents[1]
    module_path = repo_root / "vsa_recovery_sequence_casebook.py"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create audit-only 6C recovery-sequence casebook rows from saved audit JSON."
    )
    parser.add_argument("input_json", help="Path to saved audit/review JSON")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = module.build_vsa_recovery_sequence_casebook(payload)
    ...
    if args.csv_output:
        csv_text = module.render_vsa_recovery_sequence_casebook_csv(summary)
