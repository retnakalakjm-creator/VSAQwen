...
from audit.progression_role_classification import (  # noqa: E402
...
def _default_k19_dir(basket_name: str) -> Path:
    return (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Classify progression as transition/deceleration versus "
            "continuation evidence from frozen K19 causal rows."
        )
    )
    parser.add_argument(
        "--basket",
    ...
    parser.add_argument("--k19-dir", type=Path, default=None)
    parser.add_argument("--output-dir", type=Path, default=None)
    args = parser.parse_args(argv)

    basket = get_vsa_audit_basket(args.basket)
    input_dir = args.k19_dir or _default_k19_dir(basket.name)
    ...
    if artifact.requested_symbol_count != len(basket.symbols):
        raise ValueError(
            "K19 requested-symbol count does not match basket size"
        )

    audit = build_progression_role_audit(artifact)
    output_dir = args.output_dir or _default_output(basket.name)
    paths = write_progression_role_audit(audit, output_dir)
