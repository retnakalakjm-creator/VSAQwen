...
MODULE_NAME = "_pro_vsa_absorption_background_casebook"
...
def _load_casebook_module() -> Any:
    ...
    module_path = repo_root / "vsa_absorption_background_casebook.py"
    spec = importlib.util.spec_from_file_location(MODULE_NAME, module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load 6D absorption-background module from {module_path}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build audit-only 6D absorption-background casebook rows from saved audit/replay JSON."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Saved rows before a seed row to use for prior supply/background context",
    )
    parser.add_argument(
        "--lookahead-rows",
        type=int,
    ...
    parser.add_argument(
        "--production-only",
        action="store_true",
        help="Ignore diagnostic hint rows and seed only from production absorption/stopping-volume codes",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = module.build_vsa_absorption_background_casebook(
        payload,
    ...
    if args.csv_output:
        csv_text = module.render_vsa_absorption_background_casebook_csv(summary)
