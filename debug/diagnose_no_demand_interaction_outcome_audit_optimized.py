"""NO_DEMAND interaction-outcome audit.

Analysis-only. Uses the frozen NO_DEMAND candidate population and point-in-time
production evidence. The target event is excluded from its own interaction set.
Interactions are grouped into clean / other_supply / other_demand / explicit
multi-event combinations, then compared by forward outcome.
"""
...
def classify_interaction(items: list) -> tuple[str, tuple[str, ...]]:
    ...
    supply = sorted({item.code.name for item in others if item.category == EvidenceCategory.SUPPLY})
    demand = sorted({item.code.name for item in others if item.category == EvidenceCategory.DEMAND})
...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        try:
            ...
            for index in indices:
                ...
                if any(item.category == EvidenceCategory.SUPPLY and item.code is not TARGET_CODE for item in same_bar):
                    supply_interaction_events += 1
                if any(item.category == EvidenceCategory.DEMAND and item.code is not TARGET_CODE for item in same_bar):
    ...
    print("NO_DEMAND INTERACTION OUTCOME AUDIT")
    print({
        ...
        "groups": {
            name: {
                ...
                "positive_decisive_rate": (
                    sum(r > 0.0 for r in values) / sum(r != 0.0 for r in values)
    ...
    print({"interaction_combinations": dict(sorted(interactions.items()))})
