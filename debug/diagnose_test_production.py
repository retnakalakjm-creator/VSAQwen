...
def inspect_symbol(symbol: str) -> list[dict]:
    ...
    for index in range(MIN_REPLAY_BARS, len(metrics)):
        ...
        tests = tuple(
            ...
            if item.code is EvidenceCode.TEST
        ...
        rows.append(
            {
                ...
                "strengths": [float(item.strength) for item in tests],
                "weights": [float(item.weight) for item in tests],
            }
        )
...
def main() -> None:
    ...
    print("TEST PRODUCTION REPLAY SUMMARY")
    print(
        {
            "symbols_requested": len(symbols),
            "symbols_with_events": len({row["symbol"] for row in events}),
            "production_test_events": len(events),
            "outcome_classes": outcomes,
            "failures": failures,
        }
    )

    print("TEST PRODUCTION REPLAY EVENTS")
    for row in events:
        print(row)
