...
Uses direct point-in-time semantic predicates and groups overlap with
same-bar supply/demand evidence. No production registration or scoring change.
"""
...
def _audit_symbol(symbol: str) -> Counts:
    ...
    for i in range(1, len(metrics)):
        ...
        # Supply-side overlap proxies, using the current detector semantics where possible.
        supply_hits = {
            "SUPPLY_COMING_IN_LIKE": direction == Direction.DOWN and is_high_volume(bar),
            "INCREASING_SUPPLY_LIKE": direction == Direction.DOWN and volume > VolumeClass(int(prev[COL_VOLUME_CLASS])),
            "HIDDEN_SUPPLY_LIKE": _hidden_supply(metrics, i),
            "UPTHRUST_LIKE": direction == Direction.UP and is_very_high_volume(bar) and is_above_average_spread(bar) and is_weak_close(bar),
            "NO_DEMAND_LIKE": direction == Direction.UP and is_low_volume(bar) and is_narrow_spread(bar),
            "BUYING_CLIMAX_LIKE": direction == Direction.UP and is_very_high_volume(bar) and is_above_average_spread(bar) and is_weak_close(bar),
        }
        for name, hit in supply_hits.items():
            if hit:
                setattr(out, name.lower().replace("_like", "_like"), getattr(out, name.lower().replace("_like", "_like"), 0) + 1)
        # Correct explicit fields for output.
        if supply_hits["SUPPLY_COMING_IN_LIKE"]: out.supply_coming_in_like += 1
        if supply_hits["INCREASING_SUPPLY_LIKE"]: out.increasing_supply_like += 1
        if supply_hits["HIDDEN_SUPPLY_LIKE"]: out.hidden_supply_like += 1
        if supply_hits["UPTHRUST_LIKE"]: out.upthrust_like += 1
        if supply_hits["NO_DEMAND_LIKE"]: out.no_demand_like += 1
        if supply_hits["BUYING_CLIMAX_LIKE"]: out.buying_climax_like += 1
        if any(supply_hits.values()):
            out.supply_conflict += 1

        # Demand-side interactions that can coexist or overlap on this same bar.
        stopping = (
        ...
        no_supply = direction == Direction.DOWN and is_low_volume(bar) and is_narrow_spread(bar)
        shakeout_like = (
            direction == Direction.DOWN
...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        try:
            c = _audit_symbol(symbol)
            symbols_with_results += 1
            for field in totals.__dataclass_fields__:
                setattr(totals, field, getattr(totals, field) + getattr(c, field))
    ...
    print({
        ...
        "aggregate_supply_conflicts": {
            ...
            "HIDDEN_SUPPLY_LIKE": totals.hidden_supply_like,
