"""Optimized same-bar interaction audit for NO_DEMAND.

Analysis-only. Replays the frozen NO_DEMAND candidate population and uses the
production EvidenceEngine emission set at the target bar. The target
NO_DEMAND evidence is excluded from conflict classification so an event cannot
...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        try:
            ...
            for index in indices:
                ...
                supply_codes = sorted(
                    {item.code.name for item in same_bar if item.code in SUPPLY_CODES}
                )
                demand_codes = sorted(
                    {item.code.name for item in same_bar if item.code in DEMAND_CODES}
                )

                if supply_codes or demand_codes:
                    interaction_events += 1

                # NO_DEMAND is already a supply-side conflict concept. Do not
                # classify the target itself as a conflict.
                ...
                combination = []
                if supply_codes:
                    combination.append("supply:" + "+".join(supply_codes))
                if demand_codes:
                    combination.append("demand:" + "+".join(demand_codes))
                combinations[" | ".join(combination) if combination else "clean"] += 1
    ...
    if event_total != EXPECTED_EVENTS:
        failures_out.append({
            "scope": "target_events",
            "error": f"expected {EXPECTED_EVENTS} NO_DEMAND emissions, got {event_total}",
        })
    ...
    print("NO_DEMAND INTERACTION AUDIT")
    print({
        ...
        "aggregate_supply_interactions": dict(sorted(aggregate_supply.items())),
        "demand_interaction_events": demand_interaction_events,
        "aggregate_demand_interactions": dict(sorted(aggregate_demand.items())),
