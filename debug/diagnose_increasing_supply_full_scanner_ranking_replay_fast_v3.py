"""Fast INCREASING_SUPPLY scanner ranking replay v3.

Weight-independent phase:
- identify frozen cheap-candidate dates;
- build one point-in-time TrendAnalyzer + EvidenceEngine result per cheap bar;
- derive structural qualification history once per symbol.

Weight-dependent phase:
- override the live production scoring map for INCREASING_SUPPLY;
- reuse cached point-in-time trend/evidence snapshots;
- run ScannerEngine.evaluate() for each tested weight.

Important: ProfessionalScoringEngine reads config.SUPPLY_EVIDENCE_WEIGHTS,
not Evidence.weight, so this audit changes the live config mapping only
inside the counterfactual loop and restores it in finally.
"""
...
def build_structural_history(metrics: pd.DataFrame, full_trend) -> list[EvidenceResult]:
    """Build chronological structural-progression events for qualification."""
    ...
    return [EvidenceResult(context=None, evidence=(event,)) for event in events]
...
def main() -> None:
    ...
    configured_weight = float(original_weights[TARGET_CODE])
    ...
    try:
        # Phase 1: fully independent of the tested weight.
        for symbol in SYMBOLS:
            try:
                ...
                full_metrics_trend = TrendAnalyzer().analyze(metrics)
                structural_history = build_structural_history(metrics, full_metrics_trend)
        ...
        print("INCREASING SUPPLY FULL SCANNER RANKING REPLAY FAST V3")
        print({
            ...
            "weight_source": "config.SUPPLY_EVIDENCE_WEIGHTS",
            "failures": failures,
        ...
        for weight in WEIGHTS:
            ...
            for symbol, data in prepared.items():
                ...
                for index in data["events"]:
                    trend, target_result = snapshots[index]
