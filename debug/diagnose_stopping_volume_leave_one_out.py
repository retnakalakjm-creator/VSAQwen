"""Leave-one-symbol-out robustness audit for Stopping Volume thresholds.
...
def inspect_symbol(symbol: str) -> dict[str, list[dict]]:
    ...
    for index in range(MIN_REPLAY_BARS, len(metrics)):
        ...
        for name, volume_min, spread_min, close_min in THRESHOLD_SETS:
            if volume >= volume_min and spread >= spread_min and close_ratio >= close_min:
                results[name].append({
                    "symbol": symbol,
                    "bar_index": index,
...
def main() -> None:
    ...
    print("STOPPING VOLUME LEAVE-ONE-OUT ROBUSTNESS SUMMARY")
    print({"symbols_requested": len(symbols), "failures": failures})

    for threshold_name, *_ in THRESHOLD_SETS:
        events = by_threshold[threshold_name]
        print(f"STOPPING VOLUME LEAVE-ONE-OUT {threshold_name}")
        for excluded in symbols:
            remaining = [e for e in events if e["symbol"] != excluded]
            summary = _summarize(remaining)
            print({
