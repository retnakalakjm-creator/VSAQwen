"""Optimized SUPPLY_DRYING_UP candidate / outcome audit.

Production semantics from evidence/supply.py::_collect_supply_drying_up:
- down bar
- low volume
- narrow spread
...
The production detector emission is the semantic authority. A broader cheap
gate is used to create a frozen candidate population, then the actual
production EvidenceEngine is replayed point-in-time at the target bar.
"""
...
def cheap_candidate(metrics: pd.DataFrame, index: int) -> bool:
    """Broad cheap gate around the production drying-up definition."""
...
def main() -> None:
    ...
    print("SUPPLY_DRYING_UP CANDIDATE AUDIT")
    print({
        ...
        "status": (
            "FAIL"
            if failures or duplicate_emissions or candidate_events == 0
            else "PASS"
        ),
    })
