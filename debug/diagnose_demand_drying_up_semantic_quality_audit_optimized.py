"""Analysis-only semantic-quality audit for DEMAND_DRYING_UP.

Candidate definition matches the validated candidate audit exactly:
- bullish/up bar
- decreasing volume versus previous bar
- decreasing spread versus previous bar

Additional observations are descriptors only; no production path is changed.
"""
...
def _audit_symbol(symbol: str) -> dict:
    ...
    for index in range(21, len(metrics)):
        ...
        if float(bar[COL_LOW]) > float(previous[COL_LOW]):
            higher_low += 1
        if spread <= SpreadClass.AVERAGE:
            narrow_or_average_spread += 1
    ...
    return {
        ...
        "narrow_or_average_spread": narrow_or_average_spread,
    }
...
def main() -> None:
    ...
    both_declining = sum(item["both_declining"] for item in results)
    higher_low = sum(item["higher_low"] for item in results)
    narrow_or_average_spread = sum(item["narrow_or_average_spread"] for item in results)

    print("DEMAND DRYING UP SEMANTIC QUALITY AUDIT")
    print({
    ...
    print("DEMAND DRYING UP SEMANTIC QUALITY BY_SYMBOL")
