"""UPTHRUST candidate-population audit.

Analysis-only. Uses a broad cheap prefilter, then treats actual production
UPTHRUST emission at the target bar as the authority. Point-in-time replay is
used only for cheap candidates; no full-history weight sweep is performed.
"""
...
def main() -> None:
    ...
    for symbol in SYMBOLS:
        try:
            ...
            for index in indices:
                ...
                end = float(metrics.iloc[index + FORWARD_BARS]["close"])
                outcomes.append(0.0 if start == 0.0 else end / start - 1.0)
    ...
    print("UPTHRUST CANDIDATE AUDIT")
    print({
